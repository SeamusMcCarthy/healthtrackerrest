...
class AccountControllerTest {
    ...
    private fun addUser (name: String, email: String, password: String, height: Int, startWeight: Int, gender: Char, trainerID: Int, planID: Int): HttpResponse<JsonNode> {
        return Unirest.post(origin + "/api/users")
            .body("{\"name\":\"$name\", \"email\":\"$email\", \"password\":\"$password\", \"gender\":\"$gender\", \"height\":\"$height\", \"startWeight\":\"$startWeight\", \"trainerId\":\"$trainerID\", \"planId\":\"$planID\"}")
            .asJson()
    }
    ...
    //helper function to add a test user to the database
    private fun updateUser (id: Int, name: String, email: String, password: String, height: Int, startWeight: Int, gender: Char, trainerID: Int, planID: Int): HttpResponse<JsonNode> {
...
//            .body("{\"name\":\"$name\", \"email\":\"$email\"}")
    ...
    @Nested
        ...
        fun `getting a user by id when id exists, returns a 200 response`() {
            ...
            val addResponse = addUser(validName, validEmail, validPassword, validHeight, validWeight, validGender, validPlanID, validTrainerID)
        ...
        fun `getting a user by email when email exists, returns a 200 response`() {
            ...
            //Arrange - add the user
            addUser(validName, validEmail, validPassword, validHeight, validWeight, validGender, validPlanID, validTrainerID)
    ...
    @Nested
        inner class UpdateUsers {
            ...
            fun `updating a user when it exists, returns a 204 response`() {
                ...
                val addedResponse = addUser(validName, validEmail, validPassword, validHeight, validWeight, validGender, validPlanID, validTrainerID)
                ...
                assertEquals(204, updateUser(addedUser.id, updatedName, updatedEmail, addedUser.password, addedUser.height, addedUser.startWeight, addedUser.gender, addedUser.trainerId, addedUser.planId ).status)
            ...
            fun `updating a user when it doesn't exist, returns a 404 response`() {
                ...
                assertEquals(404, updateUser(-1, updatedName, updatedEmail, validPassword, validHeight, validWeight, validGender, validTrainerID, validPlanID ).status)
            }
        }
